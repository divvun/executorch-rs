# kernels/portable/cpu/op_reflection_pad2d.cpp

> [spec:et:def:op-reflection-pad2d.torch.executor.native.reflection-pad2d-out-fn]
> Tensor& reflection_pad2d_out( KernelRuntimeContext& ctx, const Tensor& in, executorch::aten::ArrayRef<int64_t> padding, Tensor& out)

> [spec:et:sem:op-reflection-pad2d.torch.executor.native.reflection-pad2d-out-fn]
> `reflection_pad2d.out`: reflection-pads the last two dimensions of `in` by
> `padding = [pad_left, pad_right, pad_top, pad_bottom]`. Same structure as
> `[spec:et:sem:op-reflection-pad1d.torch.executor.native.reflection-pad1d-out-fn]`
> with `n = 2`. Steps:
>
> - ET_KERNEL_CHECK: `check_padding_args(2, in, padding, out, reflection=true)`
>   (see `[spec:et:sem:padding-util.torch.executor.check-padding-args-fn]`):
>   `padding.size() == 4`, `in.dim() == 3 or 4`, `in`/`out` same dtype,
>   non-negative padded sizes, and each pad `<` the corresponding input dim
>   (reflection constraint); on failure `Error::InvalidArgument`, return `out`.
> - ET_KERNEL_CHECK: `in`/`out` same dim order; else `Error::InvalidArgument`.
> - ET_KERNEL_CHECK: `in` default dim order; else `Error::InvalidArgument`.
> - `get_padding_out_target_size(2, in, padding, ...)` (see
>   `[spec:et:sem:padding-util.torch.executor.get-padding-out-target-size-fn]`):
>   enlarges the last two dims by their left+right / top+bottom pads.
> - Resize `out`; on failure `Error::InvalidArgument`, return `out`.
> - Dispatch on `in.scalar_type()` over ALL types as CTYPE and call
>   `pad2d<CTYPE>(reflection_ix, in, out, padding)` (see
>   `[spec:et:sem:padding-util.torch.executor.pad2d-fn]` and
>   `[spec:et:sem:padding-util.torch.executor.reflection-ix-fn]`): each output
>   `(h, w)` reads input `(reflection_ix(h, in_height, pad_top),
>   reflection_ix(w, in_width, pad_left))`.
> - Returns `out`.

