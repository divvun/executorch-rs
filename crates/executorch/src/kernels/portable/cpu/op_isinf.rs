...
mod tests {
    ...
    fn test_sanity_check<T: TestFloat>() {
        ...
        let expected = tfb.make_default(vec![1, 5], vec![false, false, false, false, true]);

        let mut ctx = context();
        let ret = isinf_out(&mut ctx, &in_, &out);

        assert_tensor_eq!(out, *ret);
        assert_tensor_eq!(out, expected);
    }

    // [spec:et:sem:op-isinf.torch.executor.native.isinf-out-fn/test]
    // also verifies isinf_float (f32/Half/BFloat16 inputs widen to f32) and isinf_double
    // (the f64 input takes the double path): only the inf element must map to true.
    // [spec:et:sem:op-isinf.torch.executor.native.isinf-float-fn/test]
    // [spec:et:sem:op-isinf.torch.executor.native.isinf-double-fn/test]
    #[test]
    fn op_is_inf_test_sanity_check() {
    ...
    // [spec:et:sem:op-isinf.torch.executor.native.isinf-out-fn/test]
    #[test]
    fn op_is_inf_test_sanity_check_out_dtype() {
