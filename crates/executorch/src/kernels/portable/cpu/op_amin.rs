//! Literal port of kernels/portable/cpu/op_amin.cpp.
            ...
            &|begin: i64, end: i64| {
                for out_ix in begin..end {
                    ...
                    let val: CTYPE = plan.execute::<CTYPE, _>(
                        |v: CTYPE, min_v: CTYPE| -> CTYPE {
                            if isnan_override(v) || v < min_v {
                                v
                            } else {
                                min_v
                            }
...
mod tests {
    ...
    fn test_amin_out_invalid_dimensions<T>()
    where
        T: CppTypeToScalarType + FactoryValue + FromI64,
    {
        ...
        let mut ctx = context();
        amin_out(&mut ctx, &in_, dim_list, true, &out);
    ...
    fn test_amin_out_invalid_shape<T>()
    where
        T: CppTypeToScalarType + FactoryValue + FromI64,
    {
        let tf = TensorFactory::<T>::new();
        ...
        let mut ctx = context();
        amin_out(&mut ctx, &in_, dim_list, false, &out);
        assert_ne!(ctx.failure_state(), Error::Ok);
    }

    fn test_amin_out_dtype<T>()
    where
        T: CppTypeToScalarType + FactoryValue + FromI64,
    {
        let tf = TensorFactory::<T>::new();
        ...
        assert_tensor_close!(
            out,
            tf.make_default(vec![2, 3, 1], make_i64(&[0, 0, 1, 0, 1, 2]))
        );
        ...
        assert_tensor_close!(
            out,
            tf.make_default(vec![2, 3], make_i64(&[0, 0, 1, 0, 1, 2]))
        );
        ...
        let dim_list = ArrayRef::from_raw_parts(dims_2.as_ptr(), 2);
        amin_out(&mut ctx, &in_, dim_list, true, &out);
        assert_tensor_close!(out, tf.make_default(vec![1, 1, 4], make_i64(&[0, 1, 1, 0])));

        let out = tf.zeros_default(vec![4]);
        amin_out(&mut ctx, &in_, dim_list, false, &out);
        assert_tensor_close!(out, tf.make_default(vec![4], make_i64(&[0, 1, 1, 0])));
        ...
        assert_tensor_close!(
            out,
            tf.make_default(vec![2, 1, 4], make_i64(&[0, 1, 1, 0, 4, 1, 1, 0]))
        );
        ...
        amin_out(&mut ctx, &in_, null_dim_list, true, &out);
        assert_tensor_close!(out, tf.make_default(vec![1, 1, 1], make_i64(&[2])));

        let empty: [i64; 0] = [];
        let empty_dim_list = ArrayRef::from_raw_parts(empty.as_ptr(), 0);
        amin_out(&mut ctx, &in_, empty_dim_list, true, &out);
        assert_tensor_close!(out, tf.make_default(vec![1, 1, 1], make_i64(&[2])));

        let out = tf.zeros_default(vec![]);
        amin_out(&mut ctx, &in_, null_dim_list, false, &out);
        assert_tensor_close!(out, tf.make_default(vec![], make_i64(&[2])));

        amin_out(&mut ctx, &in_, empty_dim_list, false, &out);
        assert_tensor_close!(out, tf.make_default(vec![], make_i64(&[2])));
    }

    // PORT-NOTE: the C++ `test_amin_out_dtype<ScalarType::Bool>` specialization is
    ...
    fn test_amin_out_infinity_nan<T>()
    where
        T: CppTypeToScalarType + FactoryValue + FromF64,
    {
        ...
        assert_tensor_close!(
            out,
            tf.make_default(vec![2, 3, 1], vec![f(0.0), ninf, nan, nan, nan, nan])
        );
    }

    // [spec:et:sem:op-amin.torch.executor.native.amin-out-fn/test]
    #[test]
    fn op_amin_test_invalid_dimension_list_dies() {
        test_amin_out_invalid_dimensions::<u8>();
        test_amin_out_invalid_dimensions::<i8>();
        test_amin_out_invalid_dimensions::<i16>();
        test_amin_out_invalid_dimensions::<i32>();
        test_amin_out_invalid_dimensions::<i64>();
        test_amin_out_invalid_dimensions::<f32>();
        test_amin_out_invalid_dimensions::<f64>();
        test_amin_out_invalid_dimensions::<bool>();
    }

    // [spec:et:sem:op-amin.torch.executor.native.amin-out-fn/test]
    #[test]
    fn op_amin_test_invalid_shape_dies() {
        test_amin_out_invalid_shape::<u8>();
        test_amin_out_invalid_shape::<i8>();
        test_amin_out_invalid_shape::<i16>();
        test_amin_out_invalid_shape::<i32>();
        test_amin_out_invalid_shape::<i64>();
        test_amin_out_invalid_shape::<f32>();
        test_amin_out_invalid_shape::<f64>();
        test_amin_out_invalid_shape::<bool>();
    }

    // [spec:et:sem:op-amin.torch.executor.native.amin-out-fn/test]
    #[test]
    fn op_amin_test_mismatched_dtypes_dies() {
    ...
    // [spec:et:sem:op-amin.torch.executor.native.amin-out-fn/test]
    #[test]
    fn op_amin_test_all_real_input_output_passes() {
        test_amin_out_dtype::<u8>();
        test_amin_out_dtype::<i8>();
        test_amin_out_dtype::<i16>();
        test_amin_out_dtype::<i32>();
        test_amin_out_dtype::<i64>();
        test_amin_out_dtype::<f32>();
        test_amin_out_dtype::<f64>();
        test_amin_out_dtype::<Half>();
        test_amin_out_dtype::<BFloat16>();
    }

    // [spec:et:sem:op-amin.torch.executor.native.amin-out-fn/test]
    #[test]
    fn op_amin_test_infinity_and_nan_test() {
        test_amin_out_infinity_nan::<f32>();
        test_amin_out_infinity_nan::<f64>();
        test_amin_out_infinity_nan::<Half>();
        test_amin_out_infinity_nan::<BFloat16>();
    }
}
