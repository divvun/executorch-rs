//! Literal port of kernels/portable/cpu/op_ge.cpp.
...
// PORT-NOTE: the C++ `std::greater_equal` functor becomes a zero-sized
// `GreaterEqual` type implementing `ComparisonOp`, reproducing
// `Comparison<CTYPE_COMPUTE>()` via `GreaterEqual::apply::<CTYPE_COMPUTE>`.
struct GreaterEqual;
impl ComparisonOp for GreaterEqual {
...
// ("ge.Tensor_out" / "ge.Scalar_out") is dropped — the ported comparison
...
pub fn ge_tensor_out<'a, 'b>(
    ctx: &mut KernelRuntimeContext,
    a: &Tensor,
    b: &Tensor,
    out: &'a Tensor<'b>,
) -> &'a Tensor<'b> {
    comparison_tensor_out::<GreaterEqual>(ctx, a, b, out)
}

// [spec:et:def:op-ge.torch.executor.native.ge-scalar-out-fn]
// [spec:et:sem:op-ge.torch.executor.native.ge-scalar-out-fn]
pub fn ge_scalar_out<'a, 'b>(
    ctx: &mut KernelRuntimeContext,
    a: &Tensor,
    b: &Scalar,
    out: &'a Tensor<'b>,
) -> &'a Tensor<'b> {
    comparison_scalar_out::<GreaterEqual>(ctx, a, b, out)
}
...
mod tests {
    ...
    // Element builder: `{true,false,...}` initializer lists coerce to 0/1 for the
        ...
        OUT: CppTypeToScalarType + FactoryValue + FromCmp,
    {
        ...
        let mut ctx = context();
        ge_tensor_out(&mut ctx, &a, &bb, &out);
        assert_tensor_eq!(
            out,
            tf_out.make_default(vec![2, 2], b::<OUT>(&[true, false, true, true]))
        );
    }

    fn test_ge_scalar_out<IN, OUT>()
    where
        IN: CppTypeToScalarType + FactoryValue + FromCmp,
        OUT: CppTypeToScalarType + FactoryValue + FromCmp,
    {
        ...
        let mut ctx = context();
        ge_scalar_out(&mut ctx, &a, &other, &out);
        assert_tensor_eq!(
            out,
            tf_out.make_default(sizes, b::<OUT>(&[true, false, true, true]))
        );
    }
    ...
    where
        IN: CppTypeToScalarType + FactoryValue + FromCmp,
    {
        test_ge_scalar_out::<IN, u8>();
        test_ge_scalar_out::<IN, i8>();
        test_ge_scalar_out::<IN, i16>();
        test_ge_scalar_out::<IN, i32>();
        test_ge_scalar_out::<IN, i64>();
        test_ge_scalar_out::<IN, Half>();
        test_ge_scalar_out::<IN, BFloat16>();
        test_ge_scalar_out::<IN, f32>();
        test_ge_scalar_out::<IN, f64>();
        test_ge_scalar_out::<IN, bool>();
    }
    ...
    // [spec:et:sem:op-ge.torch.executor.native.ge-scalar-out-fn/test]
    // [spec:et:sem:comparison-op.torch.executor.native.internal.comparison-scalar-out-fn/test]
    #[test]
    fn op_ge_scalar_out_test_all_real_input_bool_output_support() {
    ...
    // [spec:et:sem:op-ge.torch.executor.native.ge-scalar-out-fn/test]
    #[test]
    fn op_ge_scalar_out_test_bool_input_dtype() {
    ...
    // [spec:et:sem:op-ge.torch.executor.native.ge-scalar-out-fn/test]
    #[test]
    fn op_ge_scalar_out_test_mismatched_in_out_shapes_dies() {
        ...
        let mut ctx = context();
        et_expect_kernel_failure!(ctx, ge_scalar_out(&mut ctx, &a, &other, &out));
    }

    // [spec:et:sem:op-ge.torch.executor.native.ge-scalar-out-fn/test]
    #[test]
    fn op_ge_scalar_out_test_dynamic_out_shape_test() {
        ...
        let mut ctx = context();
        ge_scalar_out(&mut ctx, &a, &other, &out);
        assert_tensor_eq!(out, tf.make_default(sizes, vec![1, 0, 1, 1]));
    }

    // [spec:et:sem:op-ge.torch.executor.native.ge-tensor-out-fn/test]
    // [spec:et:sem:comparison-op.torch.executor.native.internal.comparison-tensor-out-fn/test]
    #[test]
    fn op_ge_tensor_out_test_all_dtypes_supported() {
    ...
    // [spec:et:sem:op-ge.torch.executor.native.ge-tensor-out-fn/test]
    #[test]
    fn op_ge_tensor_out_test_mismatched_in_shapes_dies() {
        let tf_int = TensorFactory::<i32>::new();
        let tf_bool = TensorFactory::<bool>::new();
        ...
        let mut ctx = context();
        et_expect_kernel_failure!(ctx, ge_tensor_out(&mut ctx, &a, &bb, &out));
    }

    // PORT-NOTE: guarded in C++ by `ET_SKIP_IF(is_aten, ...)`; non-ATen build runs.
    // [spec:et:sem:op-ge.torch.executor.native.ge-tensor-out-fn/test]
    #[test]
    fn op_ge_tensor_out_test_mismatched_in_out_shapes_dies() {
        let tf_int = TensorFactory::<i32>::new();
        let tf_bool = TensorFactory::<bool>::new();
    ...
    // [spec:et:sem:op-ge.torch.executor.native.ge-tensor-out-fn/test]
    #[test]
    fn op_ge_tensor_out_test_dynamic_out_shape_test() {
        let tf = TensorFactory::<i32>::new();
        ...
        let mut ctx = context();
        ge_tensor_out(&mut ctx, &a, &bb, &out);
        assert_tensor_eq!(out, tf.make_default(vec![2, 2], vec![1, 0, 1, 1]));
    }
}
