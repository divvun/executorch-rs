//! Literal port of kernels/portable/cpu/op_replication_pad3d.cpp.

use crate::kernels::portable::cpu::util::padding_util::{
    check_padding_args, get_padding_out_target_size, pad3d, replication_ix,
};
...
pub fn replication_pad3d_out<'a, 'b>(
    ...
    crate::et_kernel_check!(
        ctx,
        check_padding_args(3, in_, padding, out, false),
        InvalidArgument,
        out
    );
    ...
    let name = "replication_pad3d.out";

    crate::et_switch_all_types!(in_type, ctx, name, CTYPE, {
        pad3d::<CTYPE, _>(&replication_ix, in_, out, padding);
    });
...
mod tests {
    ...
    fn op_replication_pad3d_out<'a, 'b>(
        input: &Tensor,
        padding: ArrayRef<i64>,
        out: &'a Tensor<'b>,
    ) -> &'a Tensor<'b> {
        let mut ctx = context();
        replication_pad3d_out(&mut ctx, input, padding, out)
    }
    ...
    // [spec:et:sem:op-replication-pad3d.torch.executor.native.replication-pad3d-out-fn/test]
    #[test]
    fn smoke_test() {
        ...
        let padding_data: [i64; 6] = [1, 2, 2, 1, 1, 0];
        let padding = ir(&padding_data);
        let out = tf_float.zeros_default(vec![1, 3, 6, 5]);
        #[rustfmt::skip]
        ...
        op_replication_pad3d_out(&self_, padding, &out);
    ...
    fn smoke_test_neg_front_pad() {
        ...
        let out_expected = tf_float.make_default(
            vec![1, 1, 2, 4],
            vec![6.0, 6.0, 7.0, 7.0, 6.0, 6.0, 7.0, 7.0],
        );
        op_replication_pad3d_out(&self_, padding, &out);
        assert_tensor_close!(out, out_expected);
    }
    ...
    fn smoke_test_neg_back_pad() {
        ...
        let padding_data: [i64; 6] = [1, 1, 1, 1, 4, -5];
        let padding = ir(&padding_data);
